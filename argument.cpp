...
void ArgumentParser::usage(char * const argv[])
{
    ...
    std::cerr << " [--fallback_action=<action> Enables the watchdog even when "
                     "disabled via the dbus interface. Perform this action "
					 "when the fallback expires.\n";
    std::cerr << " [--fallback_interval=<interval in ms> Enables the watchdog "
					 "even when disabled via the dbus interface. Waits for this "
					 "interval before performing the fallback action.\n";
