...
int main(int argc, char** argv)
{
    ...
    std::map<Watchdog::Action, std::string> action_targets;
    if (!targetParam.empty()) {
        auto target = targetParam.back();
        action_targets[Watchdog::Action::HardReset] = target;
    ...
    try
    {
        ...
        Watchdog watchdog(bus, path.c_str(), eventP, std::move(action_targets));
        // Claim the bus
