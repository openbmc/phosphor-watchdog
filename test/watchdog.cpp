...
class WdogTest : public ::testing::Test
{
  public:
    ...
    WdogTest() :
        event(io), wdog(std::make_unique<Watchdog>(
                       nullptr, TEST_PATH, event, Watchdog::ActionTargetMap(),
                       std::nullopt, milliseconds(TEST_MIN_INTERVAL).count())),
    ...
    // ASIO Adevent wrapper
    sdbusplus::asio::sd_event_wrapper event;
  ...
  protected:
    ...
    Quantum waitForWatchdog(Quantum timeLimit)
    {
        ...
        //     if (event.get()->run(sleepTime) == 0)
        //     {
        //         ret += sleepTime;
        //     }
        // }
        // wdog->timeRemaining(1);
        // std::this_thread::sleep_for(Quantum(10));
        io.run_one();
        return ret;
    }
};
...
TEST_F(WdogTest, enableWdogAndResetTo5Quantums)
{
    ...
    EXPECT_FALSE(wdog->timerEnabled());
    EXPECT_EQ(wdog->timeRemaining(), 1);
}
...
//     EXPECT_EQ(defaultInterval - Quantum(1),
//     waitForWatchdog(defaultInterval));
...
//     wdog.reset();
//     wdog = std::make_unique<Watchdog>(nullptr, TEST_PATH, event,
//                                       Watchdog::ActionTargetMap(), fallback);
//     EXPECT_EQ(primaryInterval,
//     milliseconds(wdog->interval(primaryIntervalMs)));
//     EXPECT_FALSE(wdog->enabled());
//     EXPECT_EQ(0, wdog->timeRemaining());
...
//     // Waiting default expiration
//     EXPECT_EQ(primaryInterval - Quantum(1),
//     waitForWatchdog(primaryInterval));
...
//     // fallback interval
//     EXPECT_EQ(fallback.interval,
//     wdog->timeRemaining(primaryInterval.count()));
//     EXPECT_FALSE(wdog->enabled());
...
//     EXPECT_EQ(fallbackInterval - Quantum(1),
//     waitForWatchdog(fallbackInterval));
...
//     // Setting the interval should take effect once resetTimer re-enables
//     wdog EXPECT_EQ(newIntervalMs, wdog->interval(newIntervalMs));
