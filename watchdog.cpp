...
constexpr clockid_t watchdogClock = CLOCK_MONOTONIC;

/* Return current time in usec, using CLOCK_MONOTONIC.
 * Watchdog uses CLOCK_MONOTONIC for sd-event timer.
 * There is no guarantee that std::chrono::steady_clock uses CLOCK_MONOTONIC. */
uint64_t Watchdog::now() const
{
    struct timespec ts{};
    auto r = clock_gettime(watchdogClock, &ts);
    if (r == -1)
    {
...
int Watchdog::enableTimer(bool enable)
{
