...
uint64_t Watchdog::timeRemaining() const
{
    ...
    if (detail::ServerObject<detail::WatchdogIface>::enabled())
    {
        ...
        if (enabled != SD_EVENT_OFF)
        {
            ...
            // convert to msec
            auto remainMsec = duration_cast<milliseconds>(microseconds(next) -
                                steady_clock::now().time_since_epoch());
            timeRemain = static_cast<uint64_t>(remainMsec.count());
        }
    }
...
{
    // next time_point to expire
    auto nextTp = steady_clock::now() + milliseconds(value);
    // convert to usec
    auto next = static_cast<uint64_t>(duration_cast<microseconds>
                    (nextTp.time_since_epoch()).count());
