...
class Watchdog : public WatchdogInherits
{
    public:
         ...
         *  @param[in] action_targets - map of systemd targets called on timeout
         */
        Watchdog(sdbusplus::bus::bus& bus,
                const char* objPath,
                EventPtr& event,
                std::map<Action, std::string> && action_targets =
                    std::map<Action, std::string>()) :
            WatchdogInherits(bus, objPath),
            bus(bus),
            action_targets(std::move(action_targets)),
