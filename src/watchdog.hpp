...
//         asyncTimer.expires_after(std::chrono::seconds(interval));
//         asyncTimer.async_wait(handler);
//         enabled = true;
//     }
...
class Watchdog
{
  public:
     ...
     */
    Watchdog(const std::shared_ptr<sdbusplus::asio::connection>& conn,
             const char* objPath,
             const sdbusplus::asio::sd_event_wrapper& event,
        ...
        timer(event.get(), std::bind(&Watchdog::timeOutHandler, this)),
        objPath(objPath)
    {
