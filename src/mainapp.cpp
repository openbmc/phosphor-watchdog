...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        sdbusplus::asio::sd_event_wrapper event(io);
        ...
        conn->attach_event(event.get(), SD_EVENT_PRIORITY_NORMAL);

        // Create a watchdog object
        Watchdog watchdog(conn, path.c_str(), event, std::move(actionTargetMap),
