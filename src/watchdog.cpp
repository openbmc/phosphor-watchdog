...
std::function<Watchdog::Action(const std::string&)>
    Watchdog::convertActionFromString = WatchdogBase::convertActionFromString;
...
void Watchdog::initService()
{
    ...
    watchdog->register_property("Interval", interval(),
...
{
    watchdogInterval = std::max(value, minInterval);
...
bool Watchdog::setProperty(std::string_view name, PropertyType value)
{
    if (!conn)
    {
        return false;
    ...
    return watchdog->set_property(name.data(), value);
}
